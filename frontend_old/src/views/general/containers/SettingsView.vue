...
type ColorItem = { name: string; palette: Palette | {} };
...
/* ---------- KOLORY ---------- */
const primaryColors = ref<ColorItem[]>([
    { name: 'noir', palette: {} },
...
const language = ref<string>(lang.selected);
function applyLanguage(l: string) {
    lang.setLanguage(l);
    language.value = l;
    // jeśli masz vue-i18n:
    // const { locale } = useI18n(); locale.value = l
    // jeśli chcesz też header HTTP: ustaw to w axios interceptorze
}
...
onMounted(() => {
    ...
    if (primary) {
        ...
        if (primaryColor) updateColors('primary', primaryColor);
    }
    if (surface) {
        const surfaceColor = surfaces.value.find((s) => s.name === surface);
        if (surfaceColor) updateColors('surface', surfaceColor);
    }
    if (presetCookie && (presets as any)[presetCookie]) {
        preset.value = presetCookie;
        onPresetChange();
    }
    if (menuModeCookie) {
        menuMode.value = menuModeCookie;
        onMenuModeChange();
    }
});
</script>
...
<template>
    <div class="p-4 md:p-6 lg:p-8 max-w-6xl mx-auto">
        <h1 class="text-2xl md:text-3xl font-bold mb-4">Settings</h1>
        <p class="text-muted-color mb-6">Personalizuj wygląd aplikacji i preferencje językowe.</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
                ...
                <template #content>
                    <div class="space-y-6">
                        <!-- Primary -->
                        <div>
                            <div class="text-sm text-muted-color font-semibold">Primary</div>
                            <div class="pt-2 flex gap-3 flex-wrap">
                                <button
                                    ...
                                    :class="['border-none w-6 h-6 rounded-full p-0 cursor-pointer outline-offset-2 outline-2', layoutConfig.primary === c.name ? 'outline outline-primary' : 'outline-transparent']"
                                    :style="{ backgroundColor: `${c.name === 'noir' ? 'var(--text-color)' : (c.palette as any)['500']}` }"
                                />
                            </div>
                        </div>
                        ...
                        <!-- Surface -->
                        <div>
                            <div class="text-sm text-muted-color font-semibold">Surface</div>
                            <div class="pt-2 flex gap-3 flex-wrap">
                                <button
                                    v-for="s in surfaces"
                                    :key="s.name"
                                    ...
                                    :class="['border-none w-6 h-6 rounded-full p-0 cursor-pointer outline-offset-2 outline-2', layoutConfig.surface === s.name ? 'outline outline-primary' : 'outline-transparent']"
                                    :style="{ backgroundColor: `${(s.palette as any)['500']}` }"
                                />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <div class="text-sm text-muted-color font-semibold mb-2">Preset</div>
                                <SelectButton v-model="preset" :options="presetOptions" :allowEmpty="false" @change="onPresetChange" />
                            </div>

                            <div class="flex items-end">
                                <Button :icon="isDarkTheme ? 'pi pi-moon' : 'pi pi-sun'" class="p-button-outlined w-full" label="Toggle Dark/Light" @click="toggleDarkMode" />
                            </div>
                        </div>
                        ...
                        <!-- Menu Mode -->
                        <div>
                            <div class="text-sm text-muted-color font-semibold mb-2">Menu Mode</div>
            ...
            <Card>
                ...
                <template #content>
                    <div class="space-y-4">
                        <div>
                            <div class="text-sm text-muted-color font-semibold mb-2">Application language</div>
                            <Dropdown v-model="language" :options="languageOptions" optionLabel="label" optionValue="value" placeholder="Select language" class="w-full" @change="applyLanguage(language)" />
                        </div>
                    </div>
                </template>
            </Card>
